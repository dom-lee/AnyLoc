...
dbStruct = namedtuple('dbStruct', [
    'whichSet', 'dataset', 'dbImage', 'locDb', 'qImage', 'locQ', 'numDb',
...
def parse_dbStruct(path):
    ...
    return dbStruct(whichSet, dataset, dbImage, locDb, qImage, locQ, numDb,
                    numQ, posDistThr, posDistSqThr)
    ...
    def __init__(self,
        ...
        for dataset_name in dataset_name_list:
            ...
            if dataset_name == 'gardens':
                ...
                self.images_paths = self.images_paths + list(
                    self.db_abs_paths)  #[::3]
            ...
            # oxford
                ...
                structFile = os.path.join(datasets_folder,
                                          "Oxford_Robotcar/oxdatapart.mat")
                root_dir = os.path.join(datasets_folder,
                                        "Oxford_Robotcar/oxDataPart")

                self.dbStruct = parse_dbStruct(structFile)
                self.images_paths = self.images_paths + [
                    os.path.join(root_dir, dbIm.replace(' ', ''))
                    for dbIm in self.dbStruct.dbImage
                ]  #[::3]
            ...
            # baidu
                ...
                for p in self.db_paths:
                    self.db_abs_paths.append(
                        os.path.join(self.datasets_folder, self.dataset_name,
                                     "training_images_undistort", p))

                self.images_paths = self.images_paths + list(
                    self.db_abs_paths)  #[::3]
                print("number of images in baidu dataset",
            ...
            # eiffel
                ...
                self.images_paths = self.images_paths + list(
                    self.db_abs_paths)  #[::3]
                print("number of images in eiffel dataset",
                      len(self.images_paths))
            ...
            #hawkins
                ...
                self.images_paths = self.images_paths + list(
                    self.db_abs_paths)  #[::3]
                print("number of images in hawkins dataset",
                      len(self.images_paths))
            ...
            #VPAir
                ...
                self.images_paths = self.images_paths + list(
                    self.db_abs_paths)  #[::3]
                print("number of images in VPAir dataset",
                      len(self.images_paths))
            ...
            #GNSS Tartan
                ...
                self.images_paths = self.images_paths + list(
                    self.db_abs_paths)  #[::3]
                print("number of images in GNSS Tartan dataset",
                      len(self.images_paths))
            ...
            # other vg datasets
                ...
                self.args = args
                self.dataset_name = dataset_name
                self.datasets_folder = os.path.join(datasets_folder,
                                                    dataset_name)
                ...
                if "ref" in os.listdir(self.datasets_folder):
                    ...
                    self.datasets_folder = join(self.datasets_folder, "images",
                                                split)
                    database_folder_name = "database"
                if not os.path.exists(self.datasets_folder):
                    raise FileNotFoundError(
                        f"Folder {self.datasets_folder} does not exist")

                self.resize = args.resize
                self.test_method = args.test_method

                #### Read paths and UTM coordinates for all images.
                database_folder = join(self.datasets_folder,
                                       database_folder_name)
                if not os.path.exists(database_folder):
                    raise FileNotFoundError(
                        f"Folder {database_folder} does not exist")
                self.database_paths = natsorted(
                    glob(join(database_folder, "**", "*.jpg"), recursive=True))

                if dataset_name == "pitts30k":
                    self.images_paths = self.images_paths + list(
                        self.database_paths)[::10]
                elif dataset_name == "st_lucia":
                    self.images_paths = self.images_paths + list(
                        self.database_paths)[::2]
                else:
                    self.images_paths = self.images_paths + list(
                        self.database_paths)  #[::1]
