...
class Baidu_Dataset(torch.utils.data.Dataset, CustomDataset):
    ...
    def __init__(self,
                 args,
                 datasets_folder=prog_args.data_vg_dir,
                 dataset_name="baidu_datasets",
                 split="train",
        ...
        for idx, db_gt_file_rel in enumerate(self.db_gt_paths):
            ...
            db_gt_file = os.path.join(self.datasets_folder, self.dataset_name,
                                      "training_gt", db_gt_file_rel)
        ...
        for idx, q_gt_file_rel in enumerate(self.q_gt_paths):
            ...
            q_gt_file = os.path.join(self.datasets_folder, self.dataset_name,
                                     "query_gt", q_gt_file_rel)
        ...
        if use_ang_positives:
            ...
            self.dist, self.soft_dist_positives_per_query = knn.radius_neighbors(
                self.q_gt_arr, radius=self.dist_thresh, return_distance=True)
            ...
            self.dist, self.soft_dist_positives_per_db = knn.radius_neighbors(
                self.db_gt_arr, radius=self.dist_thresh, return_distance=True)

            self.soft_positives_per_db = []

            for i in range(len(self.db_gt_arr)):  #iterate over all q_gt_array
                self.ang_dist = []
                for j in range(len(self.soft_dist_positives_per_db[i])
                               ):  #iterate over all positive queries
    ...
    def _test_query_transform(self, img):
        ...
        if self.test_method == "single_query":
            ...
            processed_img = T.functional.center_crop(processed_img,
                                                     self.resize)
            assert processed_img.shape[1:] == torch.Size(
                self.resize), f"{processed_img.shape[1:]} {self.resize}"
...
if __name__ == "__main__":
    ...
    print(len(vpr_ds.soft_positives_per_query[q_idx])
          )  #,len(vpr_ds.soft_dist_positives_per_query[q_idx]))
    for i in range(len(vpr_ds.soft_positives_per_query[q_idx])):
        print(vpr_ds.db_paths[vpr_ds.soft_positives_per_query[q_idx][i]],
              vpr_ds.dist[q_idx][i])
