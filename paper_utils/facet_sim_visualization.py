...
def plot_facet_sim(dump_file_paths: list,
                   output_path: str,
    ...
    for i, dump_file in enumerate(dump_file_paths):
        ...
        if i in [1, 2] and not layer_ablation:
            ...
            simg_np = cv2.resize(simg_np, (512, 384),
                                 interpolation=cv2.INTER_CUBIC)
            timg_np = cv2.resize(timg_np, (512, 384),
                                 interpolation=cv2.INTER_CUBIC)
            pix_loc = (pix_loc[0], pix_loc[1] * 384 / 320)
            for sim_key in sims.keys():
                sims[sim_key] = cv2.resize(sims[sim_key], (512, 384),
                                           interpolation=cv2.INTER_CUBIC)
        ...
        for facet in ["Key", "Query", "Value", "Token"]:
            ax[i, 1].plot(mlocs[facet.lower()][1],
                          mlocs[facet.lower()][0],
                          markermap[facet.lower()],
                          label=facet,
                          c=colormap[facet.lower()],
                          **mp)
        # Set ticks to be off
        ...
        for facet_idx, facet in enumerate(["key", "query", "value", "token"]):
            ax[i, facet_idx + 2].imshow(
                (((sims[facet] + 1) / 2) * 255).astype(np.uint8),
                vmin=0,
                vmax=255,
                cmap="jet")
        ...
        if i == len(dump_file_paths) - 1:
            y_loc = 0
            pad = -23
            ax[i, 0].set_title("Database Image",
                               y=y_loc,
                               pad=pad,
                               fontdict=font_dict)
            ax[i, 1].set_title("Query Image",
                               y=y_loc,
                               pad=pad,
                               fontdict=font_dict)
        if layer_ablation:
...
def dump_file_data_sim():
    ...
    dump_fps = [
        '/ocean/projects/cis220039p/nkeetha/data/vlm/found/facet_similarity/facet-sim/sense_baidu-mall/I455Q-457Q_Px410_Py115.gz',
...
def dump_file_data_ablation():
    ...
    dump_fps = [
        '/ocean/projects/cis220039p/nkeetha/data/vlm/found/facet_similarity/layers_day-night_Oxford/L7/I1D-1Q_Px555_Py200.gz',
...
if __name__ == '__main__':
    ...
    plot_facet_sim(dump_file_paths,
                   output_path,
                   layer_ablation=True,
