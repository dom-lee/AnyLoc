...
from utilities import VLAD, get_top_k_recall, seed_everything, reduce_pca
...
class LocalArgs:
    ...
    prog: ProgArgs = ProgArgs(wandb_proj="Dino-v2-Descs", wandb_group="VLAD-Descs")
    ...
    top_k_vals: List[int] = field(default_factory=lambda: list(range(1, 21, 1)))
...
def build_vlads(
    ...
    if largs.cache_vlad_descs:
        cache_dir = (
            f"{largs.prog.cache_dir}/vlad_descs/Dino/"
    ...
    dino = DinoV2ExtractFeatures(
        largs.model_type, largs.desc_layer, largs.desc_facet, device=device
    )
    if verbose:
    ...
    if vlad.can_use_cache_ids(db_img_names):
        if verbose:
            print("Valid cache found, using it")
        db_vlads = vlad.generate_multi([None] * len(db_indices), db_img_names)
    else:
        if verbose:
        ...
        db_vlads: torch.Tensor = vlad.generate_multi(full_db, db_img_names)
        del full_db
    if verbose:
    ...
    if vlad.can_use_cache_ids(qu_img_names):
        if verbose:
            print("Valid cache found, using it")
        qu_vlads = vlad.generate_multi([None] * len(qu_indices), qu_img_names)
    else:
        if verbose:
            print("Valid cache not found, doing forward pass")
        full_qu = []
        ...
        qu_vlads: torch.Tensor = vlad.generate_multi(full_qu, qu_img_names)
        del full_qu
    if verbose:
    ...
    if vpr_distractor_ds is not None:
        ...
        try:
            ...
            if vlad.can_use_cache_ids(db_dis_img_names):
                if verbose:
                    print("Valid cache found, using it")
                db_dis_vlads = vlad.generate_multi(
                    [None] * len(db_dis_indices), db_dis_img_names
                )
            else:
                if verbose:
                    print("Valid cache not found, doing forward pass")
                ...
                full_dis_db = extract_patch_descriptors(
                    db_dis_indices, use_distractor=True
                )
                if verbose:
                    print(
                        f"Full distractor database descriptor shape: {full_dis_db.shape}"
                    )
...
def main(largs: LocalArgs):
    ...
    if ds_name == "baidu_datasets":
        vpr_ds = Baidu_Dataset(largs.bd_args, ds_dir, ds_name, largs.data_split)
    elif ds_name == "Oxford":
        ...
        vpr_ds = Hawkins(
            largs.bd_args, ds_dir, "hawkins_long_corridor", largs.data_split
        )
    elif ds_name == "VPAir":
        vpr_ds = VPAir(largs.bd_args, ds_dir, ds_name, largs.data_split)
        vpr_distractor_ds = VPAir_Distractor(
            largs.bd_args, ds_dir, ds_name, largs.data_split
        )
    ...
    if ds_name == "VPAir":
        db_vlads, qu_vlads = build_vlads(
            largs, vpr_ds, vpr_distractor_ds=vpr_distractor_ds
        )
    else:
    ...
    if largs.pca_dim_reduce is not None:
        norm_descs = (
            lambda x: torch.from_numpy(x / np.linalg.norm(x, axis=-1, keepdims=True))
            .to(db_vlads.device)
            .contiguous()
        )
